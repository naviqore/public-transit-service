...
abstract class IsolineQueryTemplate<T> {
     ...
     * <p>
     * This delegates directly to the underlying routing utility without applying
     * any time window or duration minimization logic.
     *
     ...
     * <p>
     * The algorithm repeatedly shifts the source departure/arrival times in discrete increments,
     * runs an earliest-arrival routing query for each shifted time, and keeps the fastest connection
     * per stop across all iterations.
     * <p>
    ...
    private Map<String, org.naviqore.raptor.Connection> runForShortestDurationTime(
        ...
        OffsetDateTime windowLimit = timeIncrementor.apply(time, Duration.ofSeconds(queryConfig.getTimeWindowDuration()));
     ...
     * <p>
     * This represents the walking or access time to reach a source stop before (ARRIVAL)
     * or after (DEPARTURE) the reference time and is later added to the routing duration.
     *
     ...
     */
    private Map<String, Duration>  calculateCostsPerSourceStop(OffsetDateTime referenceTime, Map<String, OffsetDateTime> sourceStops) {
        return sourceStops.entrySet().stream()
     ...
     * <p>
     * The increment is chosen such that the next routing iteration starts just after
     * the earliest relevant trip time found in the current iteration, ensuring progress
     * while avoiding redundant queries.
     *
     ...
     */
    private Duration getNextTimeIncrement(Map<String, OffsetDateTime> sourceStopTimes, Map<String, org.naviqore.raptor.Connection> isolines) {
        return isolines.values().stream()
                .map(connection -> {
    ...
    /**
     * Compares two connections and determines whether the new connection represents
     * a shorter total travel time.
     * <p>
     * Total travel time is defined as the RAPTOR-reported duration plus the source stop
     * access cost.
     *
     ...
     */
    private boolean newConnectionIsFaster(org.naviqore.raptor.Connection newConnection, org.naviqore.raptor.Connection referenceConnection, Map<String, Duration> costPerSourceStop){
        Duration newTravelTime = Duration.ofSeconds(newConnection.getDurationInSeconds()).plus(costPerSourceStop.get(getSourceStopIdFromConnection(newConnection)));
        Duration previousBestTravelTime = Duration.ofSeconds(referenceConnection.getDurationInSeconds()).plus(costPerSourceStop.get(getSourceStopIdFromConnection(referenceConnection)));
     ...
     * <p>
     * For DEPARTURE queries, this is the departure time of the first leg.
     * For ARRIVAL queries, this is the arrival time of the last leg.
     *
    ...
    private OffsetDateTime getTripTimeFromConnection(org.naviqore.raptor.Connection connection) {
        return timeType == DEPARTURE ?
                connection.getLegs().getFirst().getDepartureTime():
                connection.getLegs().getLast().getArrivalTime();
    }

    /**
     * Determines the source stop ID associated with a connection.
     * <p>
     * For DEPARTURE queries, this is the {@code fromStopId} of the first leg.
     * For ARRIVAL queries, this is the {@code toStopId} of the last leg.
     *
     * @param connection the RAPTOR connection
    ...
    private String getSourceStopIdFromConnection(org.naviqore.raptor.Connection connection){
        return timeType == DEPARTURE ?
                connection.getLegs().getFirst().getFromStopId():
                connection.getLegs().getLast().getToStopId();
    }

    /**
     * Shifts all source stop times by a given increment.
     * <p>
     * The direction of the shift (forward or backward in time) depends on the provided
     * time incrementor.
     *
     ...
     */
    private Map<String, OffsetDateTime> incrementSourceStopTimes(Map<String, OffsetDateTime> sourceStopTimes, Duration increment, BiFunction<OffsetDateTime, Duration, OffsetDateTime> timeIncrementor) {
        return sourceStopTimes.entrySet().stream().collect(Collectors.toMap(
                Map.Entry::getKey,
     ...
     * <p>
     * For DEPARTURE queries, the current time must be before the window limit.
     * For ARRIVAL queries, the current time must be after the window limit.
     *
    ...
    private boolean currentTimeIsRelevant(OffsetDateTime currentTime, OffsetDateTime windowLimit) {
        return timeType == DEPARTURE && currentTime.isBefore(windowLimit) || timeType == ARRIVAL && currentTime.isAfter(windowLimit);
    }
