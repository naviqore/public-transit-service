...
public class WalkTransferGenerator implements TransferGenerator {
     ...
     * @param maxWalkDistance     Maximum walking distance between stops (search radius for stops) in meters.
     */
    ...
    public List<Transfer> generateTransfers(GtfsSchedule schedule) {
        ...
        List<Transfer> transfers = stops.values().parallelStream().flatMap(fromStop -> {
    ...
    private Transfer createTransfer(Stop fromStop, Stop toStop) {
        ...
        return new Transfer(fromStop, toStop, transferDuration);
    }
