...
public interface PublicTransportService {
    @NotNull List<StopTime> getNextDepartures(@NotNull Stop stop, @NotNull LocalDate from, @Nullable LocalDate until, int maxDepartures);

    @NotNull List<Stop> getStops(@NotNull String like, @NotNull SearchType searchType);

    @Nullable Stop getClosestStop(@NotNull Location location, int searchRadius);

    @NotNull List<Stop> getClosestStops(@NotNull Location location, int searchRadius, int maxStops);
